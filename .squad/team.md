# Squad Team

> Plan_for_Chicago_2030

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Freamon | Lead | [charter](.squad/agents/freamon/charter.md) | active |
| McNulty | Geo Developer | [charter](.squad/agents/mcnulty/charter.md) | active |
| Bunk | Data Engineer | [charter](.squad/agents/bunk/charter.md) | active |
| Greggs | Tester | [charter](.squad/agents/greggs/charter.md) | active |
| Scribe | Session Logger | [charter](.squad/agents/scribe/charter.md) | active |
| Ralph | Work Monitor | [charter](.squad/agents/ralph/charter.md) | active |

## Project Context

- **Project:** Plan_for_Chicago_2030
- **User:** Aaron
- **Created:** 2026-03-30
- **Stack:** Python, GeoPandas, Folium, Pandas, Shapely, pytest
- **Description:** Geospatial analysis for Chicago urban planning (2030). Analyzes zoning, property values, transit walkability, and land use scenarios. Key focus: Soldier Field area and South Loop.
- **Constraint:** Do NOT download new external data — all data is in the repo (data/, cache/).
